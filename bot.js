...
bot.onText(/^spam (\d)+$/i, function(msg) {
    if (
        ...
        msg.reply_to_message.text !== replies.bot_ready_reply
    ) {
        return
    }
    ...
    if (msg.from.id == process.env.OWNER) {
        request.delete(
            ...
            (err, resp, body) => {
                ...
                } else if (resp && resp.statusCode === 202) bot.sendMessage(msg.chat.id, 'Restarting the bot')
            },
        )
    }
})
...
async function spam(id, times, string, originalMessage) {
    ...
    for (var i = 1; i <= times; i++) {
        bot.sendMessage(id, string + APPEND_TEXT + i).catch(ignored => {})
...
bot.onText(/^\/newReq (.+)/i, function(msg) {
    ...
    var req = text.slice(text.indexOf(' ') + 1)
    req = req.replace(/'/g, "''")
    var from
    ...
    var query =
        'INSERT INTO requests (chat_id, user_id, req, from_name) VALUES (' +
        chatId +
        ', ' +
        msg.from.id +
        ", '" +
...
bot.onText(/^\/allReqs/i, function(msg) {
    ...
    pool.query(query, (err, result) => {
        ...
        if (result && result.rows && result.rows[0]) {
            ...
            if (items) {
                tools.deleteMsg(bot, msg, 3000)
                bot.sendMessage(msg.chat.id, items).then(m => {
                    tools.deleteMsg(bot, m, deleteDelay)
                })
            }
        } else {
            bot.sendMessage(msg.chat.id, 'There are no requests for this group.').then(m => {
                tools.deleteMsg(bot, m, deleteDelay)
            })
        }
    })
    ...
    status.then(
        result => {
            if (msg.from.id != process.env.OWNER && result.status !== 'creator' && result.status !== 'administrator') {
                ...
                var query =
                    "INSERT INTO tags (id, tag, message) VALUES ('" +
                    msg.chat.id +
                    "','" +
                    tag.toLowerCase() +
                    "','" +
    ...
    bot.getChatMember(msg.chat.id, msg.from.id).then(
        result => {
            if (msg.from.id != process.env.OWNER && result.status !== 'creator' && result.status !== 'administrator') {
                ...
                var query = "DELETE FROM tags WHERE  id = '" + msg.chat.id + "' AND tag = '" + tag.toLowerCase() + "'"
                pool.query(query, (err, result) => {
                    if (!err) {
...
function replyToTag(msg) {
    ...
    var query = "SELECT message FROM tags WHERE id='" + msg.chat.id + "' AND ("
    ...
    if (msg.new_chat_member.id == process.env.BOT_ID) {
        ...
        pool.query(query, (err, result) => {
            if (err || !result || !result.rows || !result.rows[0]) {
                bot.sendMessage(msg.chat.id, replies.leaving_chat).catch(() => {})
