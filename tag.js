...
function notifyUser (bot, user, msg, silent) {
  ...
  } else if (user.toFixed) { // user is a number, already the id
...
function onMessage (bot, msg) {
  ...
  if (msg.text && msg.entities) {
    ...
    for (var i in msg.entities) {
      ...
      } else if (entity.user) { // Users without username
        notifyUser(bot, entity.user.id, msg, false);
      }
    }
  } else if (msg.caption) { // Images/media captions
    var matched = msg.caption.match(/@[a-z0-9]*/gi);
    for (var c in matched) {
      var user = matched[c].trim().substring(1).toLowerCase();
...
// module.exports.onMessage = (bot, msg) => { onMessage(bot, msg); };
// module.exports.onCallback = (bot, call) => { onCallback(bot, call); };
// module.exports.onInfo = (bot, msg) => { onInfo(bot, msg); };
// module.exports.onStart = (bot, msg) => { onStart(bot, msg); };
